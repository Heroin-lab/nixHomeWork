let bigData = [{"id":1,"firstName":"Osmond","surName":"Kieran","sex":"Male","age":26,"isVegetarian":true,"langCount":8},
...
window.localStorage.setItem('Array', JSON.stringify(bigData));
...
function getInfoAbout() {
  ...
  let boys = 0
  let girls = 0
  const bigArray = JSON.parse(window.localStorage.getItem('Array'))
  
  for (let i=0; i < bigArray.length; i++) {
      midAge += bigArray[i].age
      
      if (bigArray[i].sex === 'Male') {
      	boys++
      } else {
      	girls++
      }
      
      if (maxLang < bigArray[i].langCount) {
      	maxLang = bigArray[i].langCount
      }
  }
  midAge /= bigArray.length
  return {midAge, boys, girls, maxLang}
}

console.log(getInfoAbout())

// 2

const countryString = 'Австрия, Бельгия, Болгария, Великобритания, Венгрия, Германия, Греция, Дания, Ирландия, Испания, Италия, Кипр, Латвия, Литва, Люксембург, Мальта, Нидерланды, Польша, Португалия, Румыния, Словакия, Словения, Финляндия, Франция, Хорватия, Чехия, Швеция и Эстония'

const countryArray = countryString.split(', ')


document.getElementById('btn').addEventListener('click', dataValidator)
...
function dataValidator() {
  ...
  if (input.value.length === 0) {
   logger.innerText = 'Empty field please type something!'
   input.style.border = 'solid 1px red'
   return
  }
  
  if (input.value.length < 4) {
  	logger.innerText = 'To short name of country!'
    input.style.border = 'solid 1px red'
    return
  }
  
  if (input.value.charAt(0) != input.value.charAt(0).toUpperCase()) {
  	logger.innerText = 'First letter need to be uppercase!'
    input.style.border = 'solid 1px red'
    return
  }
  
  for (let i=0; i < countryArray.length; i++) {
  	if (input.value == countryArray[i]) {
    	logger.innerText = 'Counrty name accepted!'
      logger.style.color = 'green'
      input.style.border = 'solid 1px gray'
    	return
    }
    logger.innerText = 'Counrty with this name is not exist!'
    input.style.border = 'solid 1px red'
  }
}